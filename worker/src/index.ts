...
import { spawn, execSync, ExecSyncOptions } from 'node:child_process';
...
function detectHardware(): { hardware: string; gpuName?: string; vramMb?: number } {
  const result: { hardware: string; gpuName?: string; vramMb?: number } = {
  ...
  try {
    ...
    const output = execSync('nvidia-smi --query-gpu=name,memory.total --format=csv,noheader,nounits 2>/dev/null', {
      timeout: 5000,
      encoding: 'utf-8',
    } as ExecSyncOptions).toString().trim();
...
function runOllamaInference(model: string, prompt: string, maxTokens: number): Promise<InferenceResult> {
...
function runOllamaEmbedding(model: string, text: string): Promise<number[]> {
  return new Promise((resolvePromise, reject) => {
    try {
      const result = execSync(
        `${config.ollamaBin} embed ${model} "${text.replace(/"/g, '\\"')}"`,
        {
          timeout: config.inferenceTimeoutMs,
          encoding: 'utf-8',
      ...
      const embedding: number[] = parsed.embedding || parsed;
...
function respondToCanary(probe: CanaryProbe): string {
...
class WorkerClient {
  ...
  private currentJobs = new Map<string, { startTime: number }>();
  private stats = {
    jobsCompleted: 0,
    jobsFailed: 0,
    tokensGenerated: 0,
  ...
  private registerHandlers(hardware: ReturnType<typeof detectHardware>): void {
    ...
    socket.on('worker:registered', (data: { id: string }) => {
    ...
    // ── Incoming Job ──
    socket.on('job', async (job: JobPayload) => {
      ...
      try {
        ...
        if (this.config.type === 'native') {
          result = await runOllamaInference(
    ...
    // ── Canary Probe ──
    socket.on('canary', (probe: CanaryProbe) => {
    ...
    // ── Job Acknowledgment ──
    socket.on('job:ack', (data: { jobId: string; status: string }) => {
